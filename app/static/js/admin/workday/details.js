document.addEventListener('DOMContentLoaded', function() {
    ...
    const workday_results_section = document.getElementById("workday-results-section");
    const workday_results_restart = document.getElementById("workday-results-restart");
    ...
    for (let i = 0; i < modify_stock_buttons.length; i++) {
        ...
        button.addEventListener("click", function(){
            ...
            let workday_token = this.getAttribute("data-workday")
            ...
            modify_sale_quantity(workday_token, sale_token, mode);

        })
    }

    function modify_sale_quantity(workday_token, sale_token, mode){

        try {
            fetch("/api/modify/w/sale/quantity/", {
                method: "POST",
                body: JSON.stringify({'workday_token': workday_token, 'sale_token': sale_token, "mode": mode, "by_unit": by_unit}),
                headers: headers
            })
            .then( resp => {
                if (resp.ok){
            ...
            .then(resp => {
                ...
                if (!by_unit) {
                    ...
                    let sale_cost_td = document.querySelector("#sale_cost_"+sale_token)
                    let sale_profit_td = document.querySelector("#sale_profit_"+sale_token)

                    sale_price_td.innerHTML = "$" + (resp.new_stock * resp.data.price)
                    sale_cost_td.innerHTML = "$" + (resp.new_stock * resp.data.cost)
                    sale_profit_td.innerHTML = "$" + (resp.new_stock * (resp.data.price - resp.data.cost))
                }
                ...
                if (!workday_results_section.classList.contains("no-display")){
                    workday_results_restart.classList.remove("no-display");
                    workday_results_section.classList.add("no-display")
                }
