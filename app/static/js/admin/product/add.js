window.onload = () =>{
    ...
    const url_address_type_val = () => document.getElementById("url_addres_type").value
    const page_number_val = () =>  parseInt( document.getElementById("search_page_number").value )

    const workday_token_val = () => getValueOfElementId("workday_token_input")
    const shoppingday_token_val = () => getValueOfElementId("shoppingday_token_input")
    const client_token_val = () => getValueOfElementId("client_token_input")
    ...
    */
    function search_btn_action(){
        ...
        if (searched_product_val().length >= 3){
            search_product( searched_product_val(),)
        ...
        form_fields.classList.add("no-display")        
    }
    ...
    */      
    ...
    document.getElementById("close_search_results").addEventListener("click", () => {
        ...
        form_fields.classList.remove("no-display")
    ...
    next_search.addEventListener("click", ()=>{        
        if (!next_search.classList.contains("disabled")){
            search_product(searched_product_val(), page_number_val()+1)
        }
    })

    // Escuchador de eventos para el botón de anterior página
    previous_search.addEventListener("click", ()=>{
        if (!previous_search.classList.contains("disabled")){
            search_product(searched_product_val(), page_number_val()-1)
        }       
    })
     ...
     * @version 1.0
    */  
    function search_product(product_name, page=1){
        try {
            let workday_token = workday_token_val()
            let shoppingday_token = shoppingday_token_val()
            let client_token = client_token_val()
            let url_address_type = url_address_type_val()
            results_list.innerHTML = ""
            ...
            fetch("/api/search/product/", {
                ...
                body: JSON.stringify({
                    ...
                    'page': page,
                    'url_address_type': url_address_type
                }),
                headers: headers
            })
            .then( resp => {
